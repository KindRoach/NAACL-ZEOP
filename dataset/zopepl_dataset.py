...
class ZopePLDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        self_encoding = self.encoder.encode_pair_pl(event_tuple.self, self.pattern)
        pos_encoding = self.encoder.encode_pair_pl(event_tuple.positive, self.pattern)
        neu_encoding = self.encoder.encode_pair_pl(event_tuple.neutral, self.pattern)
        neg_encoding = self.encoder.encode_pair_pl(event_tuple.negative, self.pattern)
...
def main():
    ...
    dataset = ZopePLDataset(data_set, DataTypeEnum.Train, data_set.bert)
    unit_test(dataset)
